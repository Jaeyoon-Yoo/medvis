...
if 'Page_now' not in st.session_state:
    ...
    st.session_state.Page_now = 'Patient_page'
...
st.sidebar.title('Medical Handoff')
st.session_state.ID = st.sidebar.selectbox("Patient ID: ", st.session_state.ID_list)
...
# btn_main_page = st.sidebar.button('Main_page', on_click = lambda: st.session_state.update(Page_now='Patient_page'))
...
if st.session_state.Page_now == 'Patient_page':
    if not st.session_state.ID:
        st.title('Welcome to Medical Handoff')
        st.divider()
        st.text('Selct Patient from the left sidebar')
    else:
        ...
        st.button('back', on_click = lambda: st.session_state.update(Page_now='Patient_page', ID=False))
    ...
    if st.session_state.Detail_Page == 'basic_page':
        ...
        table_sub = st.session_state.data_df_duration[st.session_state.data_df_duration.label.isin(['Propofol'])].set_index('subject_id').loc[st.session_state.ID].sort_values(by= 'starttime')
        table_sub['starttime'] = pd.to_datetime(table_sub['starttime'])
        # table_sub = table_sub[ table_sub['starttime'] > st.session_state.date_now - datetime.timedelta(days=1)]
        fig = px.line(table_sub, x="starttime", y="amount", title='amount of urine input', labels={'x':'time', 'y':'amount'}, markers=True )
